...
import { z } from "zod";
import { BdClient, DependencyType, IssueStatus, IssueType } from "./bdClient.js";

interface WorkspaceContext {
  BEADS_WORKING_DIR?: string;
  BEADS_CONTEXT_SET?: "1";
  BEADS_DB?: string;
}
...
function getEffectiveWorkspaceRoot(workspaceRoot?: string | null): string | undefined {
...
function ensureWriteContext(workspaceRoot?: string | null): void {
...
async function findBeadsDb(workspaceRoot: string): Promise<string | undefined> {
...
(server as any).registerTool(
  "set_context",
  {
    ...
    description:
      "Set the workspace root directory for all bd operations. Call this first for multi-repo setups.",
    inputSchema: {
...
(server as any).registerTool(
  ...
  async (_args: any) => {
    const contextSet =
      workspaceContext.BEADS_CONTEXT_SET || process.env.BEADS_CONTEXT_SET || undefined;
    ...
    const workingDir =
      workspaceContext.BEADS_WORKING_DIR ?? process.env.BEADS_WORKING_DIR ?? "NOT SET";
...
(server as any).registerTool(
  ...
  async (args: any) => {
    ...
    for (const issue of issues) {
      ...
      if (issue.dependents) issue.dependents = [];
    }
...
(server as any).registerTool(
  "create",
  {
    title: "Create issue",
    description:
      "Create a new issue (bug, feature, task, epic, or chore) with optional design and dependencies.",
    inputSchema: {
      title: z.string(),
...
(server as any).registerTool(
  "update",
  {
    title: "Update issue",
    description:
      "Update an existing issue (status, priority, design, notes, etc). Status 'closed'/'open' routes to close/reopen.",
    inputSchema: {
      issue_id: z.string(),
...
(server as any).registerTool(
  "dep",
  {
    ...
    description:
      "Add a dependency between issues. Types: blocks (hard blocker), related (soft link), parent-child (epic/subtask), discovered-from (found during work).",
    inputSchema: {
      issue_id: z.string(),
      depends_on_id: z.string(),
      dep_type: z
        .enum(["blocks", "related", "parent-child", "discovered-from"])
        .optional() as z.ZodType<DependencyType>,
      workspace_root: z.string().optional(),
    } as any,
  },
  async (args: any) => {
    ...
    const type: DependencyType = dep_type ?? "blocks";
    try {
...
(server as any).registerTool(
  "stats",
  {
    ...
    description:
      "Get statistics: total issues, open, in_progress, closed, blocked, ready, and average lead time.",
    inputSchema: {
      workspace_root: z.string().optional(),
    } as any,
  },
...
(server as any).registerTool(
  "blocked",
  {
    ...
    description:
      "Get blocked issues showing what dependencies are blocking them from being worked on.",
    inputSchema: {
      workspace_root: z.string().optional(),
    } as any,
  },
...
(server as any).registerTool(
  "init",
  {
    ...
    description:
      "Initialize bd in current directory. Creates .beads/ directory and database with optional custom prefix for issue IDs.",
    inputSchema: {
...
(server as any).registerTool(
  "validate",
  {
    ...
    description:
      "Run comprehensive database health checks (orphans, duplicates, pollution, conflicts).",
    inputSchema: {
